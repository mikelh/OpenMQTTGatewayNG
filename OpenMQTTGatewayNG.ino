/*
  OpenMQTTGatewayNG  - ESP8266 or Arduino program for home automation next generation
...
void setup()
{
 ...
 //   #ifdef ZgatewaySRFB
      Serial.begin(115200); // in the case of sonoff RF Bridge the link to the RF emitter/receiver is made by serial and need TX/RX
 //   #else
...
// use D1 (5), D2 (4), D3 (0), D4 (2) for debug signalling
//pinMode(5, OUTPUT);
//pinMode(4, OUTPUT);
//pinMode(0, OUTPUT);
//pinMode(2, OUTPUT);
...
void receivingMQTT(char * topicOri, char * datacallback) {
   ...
   if (strstr(topicOri, subjectMultiGTWKey) != NULL) // storing received value so as to avoid publishing this value if it has been already sent by this or another OpenMQTTGateway
   {
      ...
      //unsigned long long data = strtoull(datacallback, NULL, 0); // we will be able to pass values > 4294967295 and hex values
       storeValue(data);
     //storeValue(data & 0xffffffff);
     //storeValue(data >> 32);
      trc(F("Data stored"));
   }
